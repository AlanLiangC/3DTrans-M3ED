...
from m3ed_pcdet.utils.tracker_utils import get_frame_track_boxes, get_motion_state, delete_tracks
from m3ed_pcdet.utils.box_fusion_utils import compute_iou, kbf
from m3ed_pcdet.utils.compatibility_utils import get_sequence_name, get_sample_idx, get_frame_id
...
from m3ed_pcdet.utils.transform_utils import world_to_ego
from m3ed_pcdet.utils.box_fusion_utils import nms
from m3ed_pcdet.datasets.augmentor.augmentor_utils import get_points_in_box

# ======== Temporal and final label refinement key functions ========

def update_ps_dict(dataset, ps_dict, min_num_pts=1):
    final_ps_dict = {}
    ...
    for idx, (frame_id, ps) in enumerate(tqdm(final_ps_dict.items(), total=len(final_ps_dict.keys()), desc='update_ps')):        
        ...
        new_boxes = final_ps_dict[frame_id]['gt_boxes']

        if new_boxes.shape[0] > 1:            
            nms_veh_mask = nms(new_boxes[:,:7].astype(np.float32), 
                            new_boxes[:,8].astype(np.float32),
                            thresh=0.5)
            new_boxes = new_boxes[nms_veh_mask]
...
def update_ps(dataset, ps_dict, tracks_veh_all, tracks_veh_static, tracks_ped=None, 
    ...
    for idx, (frame_id, ps) in enumerate(tqdm(final_ps_dict.items(), total=len(final_ps_dict.keys()), desc='update_ps')):        
        ...
        ## ----- Vehicle -----
        veh_mask = (abs(cur_gt_boxes[:,7]) == 1) & (cur_gt_boxes[:,8] >= 0.8)
        cur_veh_boxes = cur_gt_boxes[veh_mask]
        cur_veh_boxes[:,:3] -= dataset.dataset_cfg.SHIFT_COOR
        ...
        # new_veh_boxes = np.vstack([ego_trackall_boxes, ego_static_boxes])        
        ...
        if tracks_ped is not None:
            try:
                ped_mask = (abs(cur_gt_boxes[:,7]) == 2) & (cur_gt_boxes[:,8] >= 0.7)
                ...
                if cur_ped_boxes.shape[0]>0:
                    cur_ped_boxes[:,:3] -= dataset.dataset_cfg.SHIFT_COOR
                    new_ped_boxes = np.vstack([cur_ped_boxes, ego_track_boxes_ped])   
                else:
                    new_ped_boxes = ego_track_boxes_ped   
            ...
            if new_boxes.shape[0] > 1:
                nms_mask = nms(new_boxes[:,:7].astype(np.float32), 
                                new_boxes[:,8].astype(np.float32),
                                thresh=min([ped_nms_th, veh_nms_th]))
        ...
        points_1frame = get_lidar(dataset, frame_id) # 1 sweep lidar
        points_1frame[:,:3] += dataset.dataset_cfg.SHIFT_COOR
        new_boxes[:,:3] += dataset.dataset_cfg.SHIFT_COOR
        num_pts = []    
        for box in new_boxes:
            box_points, _ = get_points_in_box(points_1frame, box)
            num_pts.append(len(box_points))
...
def refine_ped_labels(tracks_ped, ped_pos_th, track_filtering_cfg, s2e_th=2, save_dir=None):
    """
    ...
    if save_dir is not None:
        save_data(tracks_ped, save_dir, name="tracks_world_ped_refined.pkl")
    return tracks_ped
